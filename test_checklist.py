...
check("avisa asimetría del par MUU/MUD",
      any("ASIMÉTRICO" in w for w in v.warnings),
...
# --- 9. Log de senales (ahora via store inyectable) -----------------------
import escalera_checklist as EC
from storage import SignalStore

with tempfile.TemporaryDirectory() as tmp:
    original = EC.store
    EC.store = SignalStore(token="", gist_id="",
                           local_path=os.path.join(tmp, "sig.jsonl"))
    try:
        vs = [
            {"symbol": "A", "verdict": "GO", "leverage_factor": 2.0,
             "streak": {"last_close": 10, "streak_days": 3, "avg_rvol_streak": 2.0,
                        "extension_atr": 1.0, "atr": 0.5},
             "liquidity": {"adv_usd": 1e7}},
        ...
        check("solo GO y WAIT en el log",
              set(df["verdict"]) == {"GO", "WAIT"}, f"{set(df['verdict'])}")
        EC.log_signals(vs)
        check("el log hace append", len(EC.load_signals()) == 4)
    finally:
        EC.store = original

...
_avg_vol_cache = {}  # sym -> (avg, expira_ts)


def avg_daily_volumes(syms):
    now = _time.time()
    missing = [s for s in syms if s not in _avg_vol_cache or _avg_vol_cache[s][1] < now]
    if missing:
        dfd = None
        try:
            dfd = yf.download(
                tickers=missing, period="1mo", interval="1d",
        ...
        for s in missing:
            ...
            try:
                if dfd is not None and not dfd.empty:
                    ...
                    if len(vols) >= 6:
                        avg = float(vols.iloc[:-1].mean())  # excluye el día en curso
                    elif len(vols) >= 1:
                        avg = float(vols.mean())
            except Exception:
                pass
            _avg_vol_cache[s] = (avg, now + 1800)
    return {s: _avg_vol_cache[s][0] for s in syms}

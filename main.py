...
def whale(min_move: float = 10.0, window: int = 5, min_rvol: float = 5.0,
    ...
    pre = pre[:scan]
    if not pre:
        result = {"candidates": [], "live": live, "premarket": premarket,
                  "scanned": 0, "universe": len(quotes)}
        store_result("whale", ckey, result)
        return result

    data = download_intraday([p[0] for p in pre], period="1d", interval="1m",
    ...
    for sym, snap, _ in pre:
        ...
        if premarket or not sess_b:
            ...
            rvol_txt = (f"{rvol:.1f}x su promedio" if rvol is not None else "—")
        ...
        if score < 3 or burst["move"] < min_move * 0.5:
            continue
        verdict = "MÁXIMA" if score == 5 else ("ALTA" if score == 4 else "MEDIA")
        out.append({
    ...
    result = {"candidates": out[:10], "live": live, "premarket": premarket,

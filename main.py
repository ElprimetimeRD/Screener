...
def radar(rvol_min: float = 2.5, move_min: float = 0.0, watch: str = "",
    ...
    for c in prelim[:40]:
        ...
        if phase == "pre" and not c["pinned"] and pm_dvol is not None:
            if pm_dvol < min_dvol or (pm_pct is not None and pm_pct < pm_min):
                continue
        if phase == "pre":
    ...
    bars_by.clear()
    gc.collect()
    result = {"data_age_sec": round(data_age) if data_age is not None else None,
...
def discover(k: float = 3.0, base: int = 20, min_vol: int = 100000, min_rel: float = 2.0,
    ...
    for s in syms:
        try:
            ...
            if last_px < min_price or (cum * last_px) < min_dvol:
                continue
            kept = []

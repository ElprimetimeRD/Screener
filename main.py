...
def whale(min_move: float = 10.0, window: int = 5, min_rvol: float = 5.0,
    ...
    data.clear()
    gc.collect()
    out.sort(key=lambda c: (-c["score"], -c["dpm"], -c["move"]))
    hits = [c for c in out if c["score"] >= 3]
    near = [c for c in out if c["score"] < 3]
    result = {"candidates": hits[:10], "near": near[:8],
...
def track(sym: str, window: int = 5):
    """Seguimiento de un símbolo YA en movimiento. No dice cuándo entrar ni
    cuándo salir: dice (a) dónde están los niveles objetivos que definen el
    riesgo y (b) cuáles de las condiciones que lo hicieron señal ya se rompieron.
    La decisión sigue siendo del operador; esto solo mide."""
    sym = (sym or "").upper().strip()
    if not _clean_symbol(sym):
        raise HTTPException(400, "símbolo inválido")
    ...
    if len(use) < 12:
        raise HTTPException(404, "sesión demasiado corta")

    last = use[-1]["close"]
    burst = burst_analysis(use, window) or {}
    ...
    # ---- niveles objetivos, con la distancia desde el precio actual ----
    ...
    levels = [l for l in [
        ...
        lvl("VWAP", vwap, "precio institucional de referencia; perderlo cambia el sesgo"),
        lvl("Origen de la ráfaga", burst.get("start_px"),
            "de aquí arrancó el impulso; abajo, el movimiento se anuló"),
        lvl("Apertura", day_open, "perderla borra toda la sesión"),
        lvl("Cierre anterior", prev_close, "abajo, el día pasa a negativo"),
    ] if l]

    # ---- señales de agotamiento, todas verificables ----
    alerts = []
    ...
    # máximos decrecientes desde el pico del día
    ...
    if health >= 70:
        ...
        state = "DETERIORÁNDOSE"
    else:
        state = "ROTO"

    result = {
        "sym": sym, "price": round(last, 2), "state": state, "health": health,

...
def radar(rvol_min: float = 2.5, move_min: float = 0.0, watch: str = "",
        ...
        if not is_pinned:
            ...
            if premarket:
                ...
                if rvol is None or rvol < rvol_min or move < move_min:
                    continue
        prelim.append({"sym": sym, "snap": snap, "pinned": is_pinned,
                       "score": (rvol or 0) + move / 10.0})
    prelim.sort(key=lambda c: (not c["pinned"], -c["score"]))

    # ---- etapa 2: velas de 1m solo para los mejores ----
    detail = max(4, min(20, detail))
    top = prelim[:detail]
    bars_by = {}
    if top:
    ...
    for c in prelim[:40]:
        ...
        if bars:
            ...
            if by_date:
                ...
                try:
                    ...
                    stair = None
                if pre and snap["avg"]:
                    pm_pct = round(sum(b["volume"] for b in pre) / snap["avg"] * 100, 1)
        ...
        cands.append({
            ...
            "chg_open": None, "elapsed": elapsed, "dvol": round(snap["dvol"]),
    ...
    bars_by.clear()
    gc.collect()
    result = {"universe": len(quotes), "candidates": cands[:40],

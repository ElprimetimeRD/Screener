...
@app.get("/api/radar")
def radar(rvol_min: float = 2.5, move_min: float = 0.0):
    ...
    for s in syms:
        try:
            ...
            if rvol >= 5 and (accel or 0) >= 1.5 and move >= 3:
                grade = "caliente"
            elif rvol >= 3 and ((accel or 0) >= 1.5 or move >= 2):
                grade = "activo"
            else:
                grade = "vigilar"
    ...
    cands.sort(key=lambda c: (order[c["grade"]], -c["rvol"]))
...
def get_universe(cap=45):
    ...
    syms, note = [], None
    for scr in ("day_gainers", "small_cap_gainers", "most_actives", "aggressive_small_caps", "day_losers"):
        try:

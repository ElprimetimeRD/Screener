...
def size_position(
    ...
    if entry <= 0 or atr <= 0 or equity <= 0:
        out["reason"] = "entrada, ATR o equity invalidos"
        return out
    ...
    **streak_params: Any,
    ...
    v.rules.append(
        Rule(
            ...
            f"{sr.streak_days} dias consecutivos (minimo {min_streak})",
        ).to_dict()
    )
    v.rules.append(
        Rule(
            "rvol_sostenido",
            bool(sr.rvol_ok),
            f"RVOL minimo de la racha {sr.min_rvol_streak:.2f}x (umbral {min_rvol}x)"
            if sr.streak_days
    ...
    v.rules.append(
        Rule(
            "no_parabolico",
            not sr.parabolic,
            f"extension {sr.extension_atr:.2f} ATR sobre la SMA",
        ).to_dict()
    )
    ...
    if liq.get("ok"):
        ...
        v.rules.append(
            Rule(
                ...
                f"(minimo ${rp['min_adv_usd']:,.0f}); boleto maximo "
                f"${max_ticket_for_liquidity(floor_usd, rp['max_pct_of_adv']):,.0f}",
            ).to_dict()
        )
        if liq.get("spike_ratio") and liq["spike_ratio"] >= 3:
            v.warnings.append(
                f"Volumen distorsionado por pico: la media es {liq['spike_ratio']:.1f}x "
                f"el percentil 25. En un dia normal la liquidez es mucho menor."
            )
    else:
...
def log_signals(verdicts: list[dict[str, Any]], path: str = SIGNAL_LOG) -> int:
    """
    Guarda una linea JSON por senal GO/WAIT. Solo lo esencial para poder
    reconstruir despues si la senal funciono: fecha, precio de referencia,
    metricas de la racha y el veredicto.
    """
                ...
                json.dumps(
                    {
                        ...
                        "atr": s.get("atr"),
                        "leverage_factor": v.get("leverage_factor"),
                        "adv_usd": (v.get("liquidity") or {}).get("adv_usd"),
                    },
                    ensure_ascii=False,
                )
                + "\n"
...
def evaluate_signals(
    ...
    if sig.empty:
        return {"ok": False, "reason": "no hay senales registradas todavia"}
    ...
    if not rows:
        return {"ok": False, "reason": "senales aun sin desenlace medible"}
    ...
    return {
        ...
        "verdict": (
            ...
            if edge is not None
            else "insuficiente para concluir"
        ),
    }

...
def compute_streak(
    ...
    if streak == 0:
        res.ok = True
        res.reason = "sin racha activa (el ultimo dia no califica)"
...
def get_earnings_risk(
    ...
    if not stamps:
        risk.note = "yfinance no devolvio fecha de earnings (dato ausente, no confirmado sin evento)"
        return risk
    ...
    if not candidates:
        risk.note = f"sin earnings en los proximos {lookahead_days} dias"
        return risk
    ...
    if days_away == 0:
        ...
        if timing == "amc":
            ...
            risk.note = "reporta HOY despues del cierre - no cargar overnight"
        elif timing == "bmo":
            risk.note = "reporto HOY antes de la apertura - regimen post-earnings"
        else:
            risk.blocks_overnight = True
            risk.note = "reporta HOY, horario desconocido - tratar como riesgoso"
    elif days_away == 1:
        risk.blocks_overnight = True
        risk.note = "reporta MANANA - no cargar overnight esta noche"
    else:
        risk.note = f"earnings en {days_away} dias"

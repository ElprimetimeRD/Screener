...
CATALOG: tuple[LeveragedETF, ...] = (
    ...
    LeveragedETF("MUU",  "MU",   +2.0, "Direxion", "Nasdaq",
                 "COLISION: existe otro MUU en TSX (LongPoint SavvyLong 2X Micron)"),
    LeveragedETF("MULL", "MU",   +2.0, "GraniteShares", "Nasdaq"),
    LeveragedETF("MUD",  "MU",   -1.0, "Direxion", "Nasdaq",
                 "ASIMETRICO: es -1x, NO -2x. No es el espejo de MUU"),
    ...
    LeveragedETF("SPAL", "SPCX", +2.0, "GraniteShares", "",
                 "SPCX es la ACCION de SpaceX, no un ETF"),
...
def check_pair(underlying: str) -> dict[str, Any]:
    ...
    if not longs:
        out["warnings"].append(f"No hay ETF 2x largo conocido para {u}")
    if not shorts:
        out["warnings"].append(
    ...
    if longs and shorts:
        ...
        if not out["symmetric"]:
            out["warnings"].append(
                f"ASIMETRICO: {best_long.ticker} es {best_long.factor:+g}x pero "
                f"{best_short.ticker} es {best_short.factor:+g}x. "
                f"Para igualar riesgo, la pata corta necesita "
                f"{abs(best_long.factor / best_short.factor):.2f}x el capital de la larga."
            )
...
if __name__ == "__main__":
    ...
    for r in pairs_report():
        if r["tradeable_pair"]:
            sym = "simetrico" if r["symmetric"] else "ASIMETRICO"
